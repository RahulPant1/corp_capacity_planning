...
### Feature 3: Executive Dashboard with Actionable Alerts & Smart Insights

> "One screen to see if your seat plan is healthy — and what to do next"
...
- **Key Insights strip**: up to 5 rule-based insight cards auto-generated from live data:
  - 🔴 Largest seat shortfall unit with priority action
  - 💡 Highest-fragmentation unit — consolidation opportunity
  - 💡 Most over-provisioned unit vs. RTO-based need, with reallocation suggestion
  - 💡 Floors below 40% utilization — consolidation opportunity
  - 📊 Estimated RTO-based seat savings as % of current allocation
...
- **AI Executive Brief** *(hidden — requires Gemini API key)*: one-click 3-paragraph CFO narrative generated by Gemini 1.5 Flash; appears only when an API key is configured in Admin or as an env var
...
3. **Executive Dashboard — KPI cards, insights & alerts** *(1:15)*
   ...
   - Point to the **Key Insights strip** — read out the top risk card (🔴) and the top opportunity card (💡)
   ...
   - *Say:* "This is the one screen leadership would see every planning cycle. The badge gives the 30-second read — [X] alerts across capacity, RTO, and other. The insights strip above it tells you exactly what to act on: the biggest shortfall, the best consolidation opportunity, and how much the optimizer could save. No reading required."
...
5. **Scenario Lab → Download Report** *(0:30)*
   ...
   - Click **"Download Scenario Report (.xlsx)"** for the data file
   - Click **"Download Boardroom Report (.pdf)"** for the print-ready PDF
   - *Say:* "Everything we just saw can be exported two ways: a full Excel data file for analysts, or a color-coded boardroom PDF — 5 pages with KPI summary, allocation tables, risk narrative, and optimization results — ready to email to leadership right now."
...
| 0:45 | Executive Dashboard | Show KPIs → read Key Insights cards → alert badge → open RTO Alerts expander | "Insights tell you what to act on; badge = 30-second read for leadership" |
...
| 4:00 | Scenario Lab | Download Excel + PDF boardroom report | "Board-ready in two formats — data file and print-ready PDF" |
